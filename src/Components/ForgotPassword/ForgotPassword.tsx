...
export default function ForgotPassword() {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.status === 200) {
          ...
          const encodedData = global.Buffer.from(
            data.email + code,
            "utf8"
          ).toString("base64");
  ...
  return (
    ...
    >
      <h1 className="text-white text-2xl font-main-font mt-[-7%]">
        Enter the email address of your account and we’ll{" "}
      </h1>
      <form
        onSubmit={handleSubmit(handleButton)}
        className="w-full max-w-md text-center mt-[2%] flex flex-col items-center"
      >
        <label className="text-2xl font-main-font text-white mr-[60%]">
          Email
        </label>
        <Inputbox
          type="text"
        ...
        <button
          type="submit"
          className="text-sm uppercase py-4 px-14 rounded-full transition font-main-font duration-500 text-white bg-black hover:bg-33B249 hover:text-black mt-2"
        >
          Submit
        </button>
      </form>
